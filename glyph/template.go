...
d3.json("/pushdown/{{.package}}", function(error, json) {
...
d3.json("/pushdown/{{.package}}", function(error, root) {
...
d3.json("/links/{{.package}}", function(json) {
  root = json;
  root.fixed = true;
  root.x = w / 2;
  root.y = h / 2 - 80;
...
function tick() {
	...
	Register("forceimports", "force directed graph weighted by importers", `<!DOCTYPE html>
<meta charset="utf-8">
<script src="http://d3js.org/d3.v3.min.js"></script>
<style>
...
// get the data
d3.csv("/csvimports/{{.package}}", function(error, links) {
